...
export const FlashcardEngine = React.memo(function FlashcardEngine({ lessonId, flashcards, onNavigateToQuiz, onNavigateToSummary }: FlashcardEngineProps) {
  ...
  const renderFormattedContent = (text: string) => {
    if (!text) return null;
    return (
      <div className="text-left w-full max-w-md mx-auto text-gray-800 text-sm sm:text-base leading-relaxed space-y-2 font-poppins">
        {text.split("\n").map((line, idx) => {
          const trimmed = line.trim();
          if (trimmed.startsWith("###")) return <h3 key={idx} className="text-base font-bold text-gray-900 mt-3 mb-1">{trimmed.replace(/^###\s*/, "")}</h3>;
          if (trimmed.startsWith("##")) return <h2 key={idx} className="text-lg font-extrabold text-gray-900 mt-4 mb-2">{trimmed.replace(/^##\s*/, "")}</h2>;
  ...
  if (!flashcards || flashcards.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center text-center p-8 bg-white border border-gray-100 rounded-[24px] shadow-[0px_10px_10px_rgba(0,0,0,0.09)] min-h-[220px] select-none w-full max-w-2xl mx-auto font-poppins">
        <div className="w-12 h-12 bg-purple-50 text-[#6949a8] border border-purple-100 rounded-xl flex items-center justify-center mb-4">
          <Presentation size={20} />
        </div>
        <h3 className="text-sm font-black text-gray-900 mb-1">Flashcards aren't available yet.</h3>
        <p className="text-xs text-gray-500 max-w-xs">Generate study materials to review concepts here.</p>
      </div>
    );
  }
  ...
  if (isSessionCompleted) {
    return (
      <div className="bg-white border border-gray-100 rounded-[24px] shadow-[0px_10px_10px_rgba(0,0,0,0.09)] p-8 md:p-10 flex flex-col items-center text-center w-full max-w-2xl mx-auto font-poppins">
        <div className="w-16 h-16 bg-emerald-50 border border-emerald-100 text-emerald-600 rounded-full flex items-center justify-center mb-6">
          <CheckCircle size={32} />
        </div>
        <h2 className="text-2xl sm:text-3xl font-bold text-gray-900 tracking-tight">Review Complete! 🎉</h2>
        ...
        <div className="grid grid-cols-2 gap-4 w-full max-w-md mt-8 bg-slate-50 border border-gray-150 p-5 rounded-[15px]">
          ...
          <div className="flex flex-col items-center">
            ...
            <span className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mt-1">Completion</span>
          </div>
        </div>
        ...
        <button onClick={restartSession} className="mt-6 text-xs font-semibold text-gray-500 hover:text-gray-700 transition-colors flex items-center gap-1.5 cursor-pointer bg-transparent border-none">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2.5 text-[10px] font-bold text-gray-400 mb-4 px-1.5 select-none">
        ...
        <div className="flex items-center gap-2 flex-1 max-w-md">
          <div className="w-full bg-gray-150 h-1.5 rounded-full overflow-hidden border border-gray-200">
            <motion.div
              className="bg-[#6949a8] h-full w-full rounded-full origin-left transform-gpu"
              initial={false}
        ...
        <div className={`relative w-full h-full transition-transform duration-500 [transform-style:preserve-3d] ${isFlipped ? '[transform:rotateY(180deg)]' : ''}`}>
          ...
          <div className="absolute inset-0 w-full h-full [backface-visibility:hidden] [transform:rotateY(180deg)] bg-white border border-gray-100 rounded-[24px] shadow-[0px_10px_10px_rgba(0,0,0,0.09)] flex flex-col items-center justify-center p-8 overflow-hidden transition-transform duration-300">
            ...
            <div className="w-full max-h-full overflow-y-auto px-4 py-8 flex items-center justify-center">
              <p className="text-base sm:text-lg font-normal text-gray-750 text-center leading-relaxed whitespace-pre-line max-w-lg font-poppins">
